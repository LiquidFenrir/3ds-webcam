...
struct thread_base {
    ...
    void sleep_for(const std::chrono::duration<Rep, Period>& length)
    {
...
class thread : public thread_base {
    ...
    {
        // fprintf(stderr, "trampoline v_arg %p\n", v_arg);
        auto const thread_arg = std::unique_ptr<P_t>(static_cast<P_t*>(v_arg));
        // fprintf(stderr, "trampoline thread_arg.get %p\n", thread_arg.get());
...
public:
    ...
    {
        static_assert(std::is_invocable_v<F, Args...>, "Can not call this function with these argument.");
        ...
        // fprintf(stderr, "thread arg.get %p\n", arg.get());
    ...
    template<class F, typename... Args, std::enable_if_t<std::is_invocable_v<F, Args...>>>
